  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100">
        ...
        <div className="relative aspect-square bg-gradient-to-br from-orange-50 to-yellow-50 p-3">
          ...
          <div className="absolute top-2 left-2">
            <span className="px-2 py-0.5 rounded-full bg-[#003366] text-white text-[9px] font-bold uppercase tracking-wider">
              {product.category}
            </span>
          </div>
          ...
          <button
            ...
            disabled={isAdding}
            className={`absolute bottom-2 right-2 h-10 w-10 rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 active:scale-90 ${
              isAdding 
                ? "bg-[#7CFC00] scale-110" 
        ...
        {/* Content */}
        <div className="p-3">
          <h3 className="font-bold text-[#003366] text-sm leading-tight mb-1 line-clamp-1">
            {product.name}
          </h3>
          <p className="text-gray-500 text-[10px] line-clamp-2 mb-2 min-h-[28px]">
            {product.description}
          </p>
          
          <DualPrice value={product.price} size="md" />
        </div>
      </div>
    </div>
  );
